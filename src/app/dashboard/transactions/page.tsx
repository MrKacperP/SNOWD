...
export default function TransactionsPage() {
  ...
  const paid = transactions.filter(item => item.status === "paid").reduce((sum,item) => sum + transactionDisplayAmount(item, isOperator),0);
  ...
  return <div className="mx-auto max-w-[1040px] space-y-5">
    ...
    <section className="grid grid-cols-2 gap-3" aria-label="Payment summary">
      <div className="rounded-2xl border border-[var(--surface-blue-border)] bg-[var(--surface-blue)] p-4 sm:p-5"><p className="text-sm text-[var(--text-secondary)]">{isOperator ? "Job payments" : "Paid"}</p><p className="mt-2 break-words text-2xl font-semibold sm:text-3xl">{loading || error ? "—" : money(paid)}</p><p className="mt-1 text-xs text-[var(--text-muted)]">CAD · cash and card</p></div>
      <div className="app-card p-4 sm:p-5"><p className="text-base text-[var(--text-secondary)]">On hold</p><p className="mt-2 break-words text-2xl font-semibold sm:text-3xl">{loading || error ? "—" : money(held)}</p><p className="mt-1 text-xs text-[var(--text-muted)]">Awaiting job completion</p></div>
    </section>
    {outstanding.length > 0 && <section className="app-card p-4 sm:p-5"><h2 className="text-xl font-semibold">To settle</h2><div className="mt-3 divide-y divide-[var(--border-color)]">{outstanding.map(job => <Link key={job.id} href={`/dashboard/jobs/${job.id}`} className="flex flex-wrap items-center justify-between gap-3 py-4"><div className="min-w-0 flex-1"><p className="font-semibold break-words">{job.address || "Snow removal"}</p><p className="mt-1 text-sm text-[var(--text-secondary)]">{job.paymentMethod === "cash" ? "Cash only · pay the operator directly" : "Card payment needed"}</p></div><span className="font-semibold">{money(jobDisplayPrice(job, isOperator)*100)} →</span></Link>)}</div></section>}
    <section className="app-card p-4 sm:p-5 sm:p-6">
      ...
      {loading ? <p role="status" className="py-8">Loading payments…</p> : list.length === 0 ? <p className="py-8 text-[var(--text-secondary)]">{error ? "Payment history is unavailable." : "No payments to show yet."}</p> : <div className="mt-4 divide-y divide-[var(--border-color)]">{list.map(item => <article key={item.id} className="payment-receipt py-4"><header className="flex flex-wrap items-center justify-between gap-3"><div className="min-w-0 flex-1"><p className="font-semibold break-words">{isOperator ? item.clientName || "Client" : item.operatorName || "Operator"}</p><p className="mt-1 text-sm capitalize text-[var(--text-secondary)]">{item.paymentMethod === "cash" ? "Cash" : "Card"} · {item.status} · {dateText(item.createdAt)}</p></div><span className="text-lg font-semibold">{money(transactionDisplayAmount(item, isOperator))}</span></header><div className="mt-3 space-y-3 border-t border-[var(--border-color)] pt-3 text-sm"><p className="break-words">{item.description || "Snow removal"}</p>{item.paymentMethod === "cash" && <p>Cash payment recorded by the operator. No card was charged.</p>}{item.chatId && <Link className="inline-flex min-h-11 items-center font-semibold underline" href={`/dashboard/jobs/${item.jobId}`}>View work order</Link>}</div></article>)}</div>}
    </section>
    <ActionGroup title={isOperator ? "Payment options & payouts" : "Payment options"}>
      ...
      <Link href="/dashboard/settings?tab=payment" className="mt-4 inline-flex min-h-11 items-center rounded-full bg-[var(--accent)] px-5 py-3 font-semibold text-white">{isOperator ? "Manage payment options" : "Payment settings"}</Link>
