...
export default function TransactionsPage() {
  ...
  return <div className="mx-auto max-w-[1040px] space-y-5">
    ...
    <section className="grid grid-cols-2 gap-3" aria-label="Payment summary">
      <div className="rounded-3xl bg-[#e8f1f5] p-5"><p className="text-base text-[#526873]">{isOperator ? "Job payments" : "Paid"}</p><p className="mt-2 break-words text-2xl font-semibold sm:text-3xl">{loading || error ? "—" : money(paid)}</p><p className="mt-1 text-xs text-[var(--text-muted)]">CAD · cash and card</p></div>
    ...
    <ActionGroup title={isOperator ? "Payment options & payouts" : "Payment options"}>
      ...
      <Link href="/dashboard/settings?tab=payment" className="mt-4 inline-flex min-h-11 items-center rounded-full bg-[#112b3b] px-5 py-3 font-semibold text-white">{isOperator ? "Manage payment options" : "Payment settings"}</Link>
