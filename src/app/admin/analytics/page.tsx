...
const COLORS = ["#2F6FED", "#7C3AED", "#10B981", "#F59E0B", "#EF4444", "#6366F1"];
      ...
      return (
        <ResponsiveContainer width="100%" height={250}>
          <LineChart {...commonProps}>
            ...
            <YAxis tick={{ fontSize: 11 }} />
            <Tooltip />
            <Line type="monotone" dataKey="revenue" stroke="#2F6FED" strokeWidth={2} dot={{ r: 3 }} />
          </LineChart>
        </ResponsiveContainer>
      );
    }
    return (
      <ResponsiveContainer width="100%" height={250}>
        <AreaChart {...commonProps}>
          ...
          <YAxis tick={{ fontSize: 11 }} />
          <Tooltip />
          <Area type="monotone" dataKey="revenue" stroke="#2F6FED" fill="#2F6FED" fillOpacity={0.15} />
        </AreaChart>
      </ResponsiveContainer>
    );
  };
          ...
          <div className="flex gap-2">
            ...
            <div className="flex bg-gray-100 rounded-lg p-0.5">
              {(["area", "bar", "line"] as const).map(t => (
                <button key={t} onClick={() => setChartType(t)} className={`px-3 py-1 text-xs rounded-md capitalize transition ${chartType === t ? "bg-white text-[#2F6FED] font-medium shadow-sm" : "text-gray-500"}`}>
                  {t}
                </button>
              ))}
            </div>
