...
export default function AdminUserDetailPage() {
  ...
  return <div className="space-y-4">
    ...
    <AdminCard className="p-5 space-y-3"><h2 className="text-lg">Uploads & profile</h2><Link className="admin-secondary inline-flex" href={`/dashboard/u/${uid}?returnTo=${encodeURIComponent(`/admin/users/${uid}`)}`}>Open public profile</Link><div className="flex flex-wrap gap-3">{[account.idPhotoUrl, ...(account.portfolioPhotos || [])].filter(Boolean).map((url, i) => <a key={i} href={url} target="_blank" rel="noopener noreferrer" className="admin-secondary">{i === 0 && account.idPhotoUrl ? 'Reopen ID upload' : `Open portfolio upload ${i + 1}`}</a>)}</div><Link href="/admin/verifications">Review verification →</Link></AdminCard>
