...
export default function NotificationsPage() {
  ...
  return <div className="space-y-4"><div className="flex flex-wrap justify-between gap-3"><div><h2 className="text-2xl">Notifications</h2><p className="text-sm text-[var(--text-muted)]">Open a record to review it. Marking a notification read does not resolve the work.</p></div><button className="admin-secondary" onClick={() => void mark()}>Mark all read</button></div><div className="flex flex-wrap gap-2">{['Unread', 'Action needed', 'All'].map(f => <button key={f} aria-pressed={f === filter} className={f === filter ? 'admin-primary' : 'admin-secondary'} onClick={() => setFilter(f)}>{f}</button>)}</div>{error && <p role="alert">{error}</p>}<AdminCard className="divide-y">{rows.map(n => <article key={n.id} className="p-4 flex flex-wrap items-start gap-3"><div className="min-w-0 basis-full break-words sm:flex-1 sm:basis-0"><p className="font-semibold">{n.title || n.message}</p>{n.preview && <p className="text-sm mt-1">{n.preview}</p>}<p className="text-xs text-[var(--text-muted)] mt-2">{n.actionRequired ? 'Action needed · ' : ''}{relativeTime(n.createdAt)}</p></div><Link href={n.href} onClick={() => void mark(n.id)} className="admin-primary">Open record →</Link>{!n.read && <button className="admin-secondary" onClick={() => void mark(n.id)}>Mark read</button>}</article>)}{!rows.length && <EmptyState title="You’re all caught up" subtitle="Matching notifications will appear here automatically." />}</AdminCard></div>;
}
