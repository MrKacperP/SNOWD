...
export default function LoginPage() {
  ...
  const handleEmailSignIn = async () => {
    ...
    try {
      ...
      if (uid) await checkProfileAndRedirect(uid);
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : "Failed to sign in";
      if (msg.includes("user-not-found") || msg.includes("wrong-password") || msg.includes("invalid-credential")) {
        setError("Incorrect email or password. Please try again.");
      } else if (msg.includes("account-exists-with-different-credential") || msg.includes("auth/wrong-password")) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (msg.includes("account-exists-with-different-credential")) {
        setError("This email is already registered with a password. Please use Continue with Email to sign in.");
  ...
  return (
    <div className="min-h-screen relative overflow-hidden">
      ...
      <div className="absolute top-1/4 -left-32 w-64 h-64 bg-[#246EB9]/20 rounded-full blur-[100px]" />
      <div className="absolute bottom-1/4 -right-32 w-64 h-64 bg-[#3A8AD4]/15 rounded-full blur-[100px]" />
              ...
              <AnimatePresence mode="wait">
                {mode === "choose" && (
                  <motion.div key="choose" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} transition={{ duration: 0.25 }} className="space-y-4">
                    ...
                    {/* Email / Password */}
                    <button onClick={() => { setMode("email"); setError(""); }} className="w-full flex items-center gap-4 p-4 bg-white/[0.06] hover:bg-white/[0.1] border border-white/10 hover:border-white/20 rounded-2xl transition-all duration-200 group">
                      ...
                      <div className="flex-1 text-left">
                        <p className="text-white font-semibold text-sm">Continue with Email</p>
                        <p className="text-white/40 text-xs">Sign in with your email &amp; password</p>
                      </div>
                      <ArrowRight className="w-4 h-4 text-white/30 group-hover:text-white/60 transition" />
                    </button>
                  </motion.div>
                ...
                {mode === "email" && (
                  <motion.div key="email" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.25 }} className="space-y-5">
                    ...
                    <button onClick={handleEmailSignIn} disabled={loading} className="w-full py-3.5 bg-[#246EB9] hover:bg-[#1B5A9A] text-white rounded-xl font-semibold transition-all duration-200 disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-lg shadow-[#246EB9]/25">
                    ...
                    <p className="text-white/30 text-xs text-center"><Shield className="w-3 h-3 inline mr-1" />Your credentials are encrypted and secure</p>
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
