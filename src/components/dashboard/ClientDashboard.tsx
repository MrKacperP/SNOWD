  ...
  return (
    <div className="app-page text-[var(--text-primary)]">
      ...
      {!loading && !loadError && nextJob && next ? <section className={`next-action job-widget job-widget--${nextJob.status}`}>
        ...
        <div className="mt-5 grid gap-3 rounded-xl bg-white/75 p-4 sm:grid-cols-3">
          ...
          <div><span className="text-xs text-[var(--text-muted)]">When</span><strong className="mt-1 block text-sm">{isAsap(nextJob) ? "As soon as possible" : scheduleText(nextJob)}</strong></div>
      ...
      <section aria-labelledby="jobs-heading">
        <div className="mb-4 flex items-center justify-between gap-4">
          <h2 id="jobs-heading" className="text-xl font-semibold tracking-tight">Current work orders</h2>
        ...
        {loadError ? (<p role="alert">Could not load jobs. <Link className="underline" href="/dashboard/jobs">Open job log to try again</Link></p>) : loading ? (
          ...
          <EmptyState title="All clear for now" description="Your next visit will appear here." actionHref="/dashboard/find" actionLabel="Book snow help" />
        ) : (
